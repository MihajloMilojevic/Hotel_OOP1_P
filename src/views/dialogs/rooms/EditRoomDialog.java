...
public class EditRoomDialog extends JDialog {
	...
	public EditRoomDialog(Room room) {
		...
		idTf.setText(String.valueOf(room.getId()));
		numberSb.setValue(room.getNumber());
		typeCb.setSelectedItem(room.getType());
		for (JCheckBox cb : additionsCbs) {
			cb.setSelected(room.getRoomAdditions().contains(RoomController.getRoomAdditionByName(cb.getText())));
		}
