...
public class EditPriceListDialog extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public EditPriceListDialog(PriceList priceList) {
		this.priceList = priceList;
		setTitle("Edit Price List");
		setModal(true);
		...
		contentPanel.setLayout(new BorderLayout(0, 0));
		{
			...
			contentPanel.add(scrollPane, BorderLayout.CENTER);
			{
				...
				{
					JLabel lblNewLabel = new JLabel("Edit Price List:");
				...
				}
				{
					...
					for (RoomType rt : RoomController.getRoomTypes()) {
						...
						try {
							spinner.setValue(priceList.getPrice(rt));
						} catch (Exception e) {
				...
				}
				{
					...
					for (RoomAddition ra : RoomController.getRoomAdditions()) {
						...
						spinner.setFont(new Font("Times New Roman", Font.PLAIN, 14));
						try {
							spinner.setValue(priceList.getPrice(ra));
						} catch (Exception e) {
							spinner.setValue(Double.valueOf(0));
						}
						GridBagConstraints gbc_spinner = new GridBagConstraints();
		...
		idTf.setText(priceList.getId());
		startDateCh.setDate(java.util.Date.from(priceList.getStartDate().atStartOfDay(java.time.ZoneId.systemDefault()).toInstant()));
		if (priceList.getEndDate() != null)
			endDateCh.setDate(java.util.Date.from(priceList.getEndDate().atStartOfDay(java.time.ZoneId.systemDefault()).toInstant()));
	}
}
